...
async def edit_start(callback: CallbackQuery, state: FSMContext):
    ...
    title, desc, link, image_id = get_vacancy_by_id(vid)
    ...
    await callback.message.answer("📸 Новая картинка или `-`")
...
async def edit_description(message: Message, state: FSMContext):
    data = await state.get_data()
    desc = data["old_description"] if message.text == "-" else message.text
